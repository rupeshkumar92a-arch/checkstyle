...
public class ExpressionOverBlockLambdaCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (body != null && body.getType() == TokenTypes.SLIST && isSingleLineLambda(ast)) {
            final DetailAST statement = findSingleStatement(body);
            if (statement != null && isConvertibleToExpressionLambda(statement)) {
                log(ast, MSG_KEY);
            }
        }
    }
    ...
    private static boolean hasReturnExpression(DetailAST literalReturn) {
        return literalReturn.findFirstToken(TokenTypes.EXPR) != null;
    }
}
